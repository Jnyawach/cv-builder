<template>
  <div class="relative" v-on-click-outside="closeDrop">
    <button @click="dropdown=!dropdown"  type="button"
            ...
            border border-gray-500 w-full rounded-xl  bg-transparent px-2 py-1
    ">
    ...
    >
      <div v-show="dropdown" class="py-2 mt-1 absolute z-10 bg-gray-700 border-gray-200 divide-y divide-gray-100 rounded-lg
        ...
        <ul class="text-sm text-white max-h-72 overflow-y-auto divide-y divide-gray-600" aria-labelledby="dropdownHoverButton">
          <li v-for="(degree, index) in degrees" :key="index" class="grid ">
            <button type="button" @click="selectOption(degree)" class="text-start text-xs font-medium w-full h-full cursor-pointer  hover:bg-sumo-700/20 p-2">
              <span class="capitalize">{{degree}}</span>
            </button>
...
import degrees from "../../assets/data/degrees.json"
//STORAGE
defineProps({
  degree:String
})
const emits=defineEmits(['updated:degree'])
const dropdown=ref(false)
const active=ref('Select')
function selectOption(option:any){
  active.value=option
  dropdown.value=false
  emits('updated:degree', option)

        ...
        <div class="bg-gray-800 lg:w-1/2 w-full rounded-xl">
          ...
          <div class="p-3.5 ">
            <form @submit.prevent="addNewHobby">
              <div class="my-5 grid">
                <div>
                  <label for="language_name" class="resume-label">Hobbies:</label>
                  <div>
                    <small>What do you like to do?</small>
                  </div>
                  <textarea placeholder="e.g. Painting, Skydiving, Singing" required rows="5" v-model="addHobbies.name" class="resume-textarea my-2"></textarea>
                </div>
...
const addHobbies=ref({
  name:'',
  id:uuidv4()
})

const addNewHobby=()=>{
  resumeStore.hobbies=addHobbies.value;
  addHobbies.value={
    name:'',
    id:uuidv4()
  }
